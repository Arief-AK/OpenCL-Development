...
int main(){
    ...
    // Create an OpenCL program from binary or source
    std::cout << "Attempting to create OpenCL program from binary" << std::endl;
    program = CreateProgramFromBinary(context, device, "HelloWorld.cl.bin");
    if(program == NULL){
        // Attempt to create from source
        std::cout << "Binary not loaded, attempting to create from source..." << std::endl;
